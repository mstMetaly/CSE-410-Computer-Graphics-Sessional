...
void rotate(Point &v, Point &axis, double angle) {
    Point temp = v;
    v = temp * cos(angle) + axis * (1 - cos(angle)) * (temp.x * axis.x + temp.y * axis.y + temp.z * axis.z)
        + Point(axis.y * temp.z - axis.z * temp.y,
                axis.z * temp.x - axis.x * temp.z,
                axis.x * temp.y - axis.y * temp.x) * sin(angle);
}

void keyboardListener(unsigned char key, int x, int y) {
    if (key == ' ') ball.active = !ball.active;
    if (key == 'r') {
        ball.pos = Point(CUBE_SIZE/2, CUBE_SIZE/2, BALL_RADIUS + 5);
        ball.vel = Point(0,0,20);
        ball.active = true;
    }
    if (key == '+' && !ball.active) ball.vel.z += 2;
    if (key == '-' && !ball.active) ball.vel.z -= 2;
    if (key == 'v') showArrow = !showArrow;

    if (key == '1') rotate(l, u, -ROTATE_ANGLE), rotate(r, u, -ROTATE_ANGLE);
    if (key == '2') rotate(l, u, ROTATE_ANGLE), rotate(r, u, ROTATE_ANGLE);
    if (key == '3') rotate(l, r, -ROTATE_ANGLE), rotate(u, r, -ROTATE_ANGLE);
    if (key == '4') rotate(l, r, ROTATE_ANGLE), rotate(u, r, ROTATE_ANGLE);
    if (key == '5') rotate(r, l, -ROTATE_ANGLE), rotate(u, l, -ROTATE_ANGLE);
    if (key == '6') rotate(r, l, ROTATE_ANGLE), rotate(u, l, ROTATE_ANGLE);
    if (key == 'w') pos = pos + u * CAMERA_SPEED;
    if (key == 's') pos = pos - u * CAMERA_SPEED;
}

void specialKeyListener(int key, int x, int y) {
    if (key == GLUT_KEY_UP) pos = pos + l * CAMERA_SPEED;
    if (key == GLUT_KEY_DOWN) pos = pos - l * CAMERA_SPEED;
    if (key == GLUT_KEY_LEFT) pos = pos - r * CAMERA_SPEED;
    if (key == GLUT_KEY_RIGHT) pos = pos + r * CAMERA_SPEED;
    if (key == GLUT_KEY_PAGE_UP) pos = pos + u * CAMERA_SPEED;
    if (key == GLUT_KEY_PAGE_DOWN) pos = pos - u * CAMERA_SPEED;
}
